...
- **A field that disappears between review and submit — done.** The presence half of the identity check used to pass over a decided field missing from the live extraction when its step was not among the steps that extraction had rendered, to avoid refusing multi-step forms whose later controls appear only after "Next". "Rendered steps" was inferred from a single pre-click extraction, so a wizard that *replaces* its later step instead of revealing it defeated the inference outright: at extraction time it is a one-step form, and every reviewed field beyond it fell through unjudged. The per-step design it "needed" turned out to be a smaller claim, not a bigger mechanism: `fillAndSubmit` fills from the pre-click extraction and nothing else, so a control absent from it is one the driver will never touch **on any step** — for a field the user decided about, that means the decision cannot reach the form and the page's default goes instead. There is no step on which that is acceptable, so the step test is gone. It does not break the case the exemption protected, because that case cannot produce the input it was skipping: a form whose later controls only exist after a click is captured the same way at *review* time, so the reviewed snapshot has no fields on those steps either. Proven against a new demo-ats fixture (`steppedConsentPage`, the one shape demo-ats could not express — every other multi-step page there renders all steps up front): reviewed with both steps in the DOM and the background-check box **unticked** by the user, served progressively at submit time, the driver used to click Next into a step where the box was still ticked and demo-ats recorded `background_check_consent: "true"` against a receipt that said declined; now it refuses pre-click with `kind: "fill"`, naming that question, and `/api/submissions` records nothing. Pinned from both sides — the three-step greenhouse form still walks, uploads and submits; the progressive rendering reviewed and submitted as itself still goes through; and a replaced step the plan decided nothing about is still not refused. What remains is not this check's business and is stated as such: a control the review never showed and the plan never answered (a pre-ticked box that appears only after "Next") is still submitted as the page ships it — a review-completeness question, not an identity one.
- **`NEEDS_RECONCILE` screenshot paths were persisted to no row — done.** The path is now merged onto the newest snapshot's `recovery_state`, on the same `screenshotPath` key `runSubmitJob`'s `submit_result` uses and the same key `listEvidenceScreenshotPaths` feeds the collector's keep-set, so a reconcile screenshot is a referenced screenshot in exactly the sense the collector already understands. Merged rather than replaced, because that row can be carrying the `submit_in_flight` marker, and post-click, so it logs rather than throws. Proven by driving the outcome: a NEEDS_RECONCILE attempt's screenshot, aged past the grace window, survives a later confirm's collector while a genuine orphan beside it is reclaimed in the same pass — without the fix the directory comes back empty.
- **Reads could straddle a demo reset commit — done.** `readWorkspaceSnapshot` resolves the workspace and runs a page's reads in one `repeatable read`, `read only` transaction; all nine dashboard pages go through it and the ten list/count helpers they call take `DbOrTx`. The bootstrap stays outside the snapshot on purpose (a re-read under `DEMO_SEED_LOCK_KEY` inside a snapshot cannot see the commit the lock waited for — the `d008a30` duplicate-workspace bug). Re-measured against the real `seedDemoWorkspace` with 1,549 polled reads: **13 empty demos before, 0 after**; and through a real `next start`, 300 requests to `/applications` during 139 back-to-back resets gave 0 non-200s and 0 empty boards.
...
- **The driver is strictly more willing to refuse than before, and no real ATS was available to probe it against — only `demo-ats`.** After the consent-bypass fix, a fill is refused unless the field has the same id, the same `fieldIdentityHash`, *and* the same `CanonicalFormField.kind`. What to watch when this first meets a real Greenhouse or Lever page: an ATS that legitimately re-renders a control into a different `type`, or reveals a field conditionally *within the same step*, will be refused; and T6's original tolerance for a field that *disappears* is deliberately reversed — since the per-step entry above, for **every** field the user decided about rather than only the ones on a step the live extraction had rendered. (That entry also retires the sentence that used to sit here: step scoping is no longer what prevents multi-step breakage, and a form whose later-step fields are replaced rather than revealed is now covered by a committed test against a `demo-ats` fixture built for it.) The widened refusal is narrow by construction — it fires only where the reviewed page and the live page disagree about a control the user answered — but it is still a refusal, and the same rule applies: if false refusals show up against a real ATS, narrow the comparison, **do not** restore the old skip. Refusing is the safe direction — a refusal is pre-click and genuinely retryable (the confirmation is handed back unspent; see the entry above), whereas the bug it replaced sent a consent the user had declined. But if false refusals show up against a real ATS, the fix is to narrow the `kind` comparison, **not** to restore the old skip.
